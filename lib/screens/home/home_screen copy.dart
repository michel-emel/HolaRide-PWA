...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // ── Greeting — professional, time-aware welcome line ────────────
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('$greet 👋',
            style: const TextStyle(
                fontSize: 20, fontWeight: FontWeight.w900, color: AppColors.textPrimary, letterSpacing: -.2)),
        const SizedBox(height: 3),
        const Text('Where are you headed today?',
  ...
  // ── Find a Ride card ──────────────────────────────────────────
  // Compact flat card: icon badge top-left, arrow top-right, then
  // title + a fixed 2-line description zone. Both cards share the
  // exact same skeleton, so they always come out the same height —
  // no fixed `height:` and no Spacer() reserving empty space.
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (trip.originLocation.isNotEmpty || trip.destinationLocation.isNotEmpty) ...[
            ...
            Row(children: [
              ...
              const SizedBox(width: 4),
              Flexible(child: Text(
                trip.originLocation.isNotEmpty ? trip.originLocation : trip.originCity,
                overflow: TextOverflow.ellipsis,
                style: const TextStyle(fontSize: 11.5, fontWeight: FontWeight.w600, color: AppColors.textPrimary))),
              const Padding(
                padding: EdgeInsets.symmetric(horizontal: 6),
                child: Icon(Icons.arrow_forward, size: 10, color: AppColors.textSecondary)),
              const Icon(Icons.location_on, size: 12, color: AppColors.danger),
              const SizedBox(width: 4),
              Flexible(child: Text(
                trip.destinationLocation.isNotEmpty ? trip.destinationLocation : trip.destinationCity,
                overflow: TextOverflow.ellipsis,
                style: const TextStyle(fontSize: 11.5, fontWeight: FontWeight.w600, color: AppColors.textPrimary))),
            ]),
          ],
